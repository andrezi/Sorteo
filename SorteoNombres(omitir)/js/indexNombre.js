...
window.onload = function(){  
	...
	names = basil.get('listFormateada');
	count = names.length; 
	// AnimarGano(); 
	var tipAnm = basil.get('pirotecniaAN') + "";
	switch(tipAnm)
	{
		case "1":
			AnimarGano(); 
...
function start(){
	if(ganadores.length < names.length){
		do{
			selected = names[Math.random() * count | 0].toUpperCase();		
		}while(ganadores.indexOf(selected) !== -1)
	  covered = selected.replace(/[^\s]/g, '_');
	  $("#NombreGanador").text(covered);
	  timer = setInterval(decode, 50);
	}
	else
	{
		$("#NombreGanador").text("Ya han gandado todos")
	}
}

function decode(){
  currentTimeSleep = currentTimeSleep + 1;
  newtext = covered.split('').map(changeLetter()).join('');
  if(newtext == covered || currentTimeSleep >= maxTimeSleep){
	  currentTimeSleep = 0;
    clearTimeout(timer);
  $("#NombreGanador").text(selected);
	ganadores.push(selected);
	covered = selected.toLowerCase();
    winnerRevealed();
    return false;
  }
  covered = newtext;
  $("#NombreGanador").text(newtext);
}

function changeLetter(){
  replacements = '0123456789ABCDEFGHIJKLMNÑOPQRSTUVWXTZabcdefghiklmnnopqrstuvwxyz%!@&*#_ ';
...
function SetWinner()
{
    $("#copa").html('<span><i class="fa fa-trophy" style="color:gold;font-size: 122px;" aria-hidden="true"></i></span>&nbsp;&nbsp;');
    $("#nCentenas").css("margin-left","auto");
    $("#ganadores").append("<span style='font-size: 36px;'><i class='fa fa-trophy' style='color:gold;' aria-hidden='true'></i>&nbsp;<span style='text-transform: capitalize; font-size: 21px;letter-spacing: 3px;'>" + covered + "</span></span>");
    $("#ganadores").append("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");
